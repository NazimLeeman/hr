...
export async function addEmployeeToRestaurant(
  restaurantId: number,
  data: {
    name: string;
    email: string;
...
export async function addEmployeesToRestaurant(
  restaurantId: number,
  data: {
    name: string;
    email: string;
...
export async function addApplicantToEmployee(
  applicantId: number,
  restaurantId: number,
  data: {
    name: string;
...
export async function updateEmployeeInformation(
  employeeId: number,
  data: {
    restaurantId?: number;
    name?: string;
...
export async function updateEmployeeById(
  ...
  try {
    ...
    if (result[0] === 1) {
      ...
      return {
        success: false,
        message: "Employee not found or no changes applied.",
      };
    }
  } catch (error) {
    console.error(error);
...
export async function deleteEmployeeLogin(employeeId: number) {
  try {
    ...
    console.log(error);
    throw new Error(
      "Error While deleteing employee login data by ID in database."
    );
  }
}
...
export async function addOwnerToRestaurant(
  restaurantId: number,
  data: {
    name: string;
    email: string;
