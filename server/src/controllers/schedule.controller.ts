...
import {
  createSchedule,
  updateScheduleForEmployee,
  findAllScheduleOfEmployee,
  findAllScheduleInRestaurant,
} from "../models/schedule/schedule.query";
...
export async function getAllScheduleOfRestaurant(
  req: AuthRequest,
  res: Response
) {
  try {
...
export async function getAllScheduleOfEmployee(
  req: AuthRequest,
  res: Response
) {
  try {
    ...
    if (restaurantId && employeeId) {
      const schedule = await findAllScheduleOfEmployee(
        employeeId,
        restaurantId
      );
      res.json({ data: schedule });
...
export async function updateScheduleOfEmployee(req: Request, res: Response) {
  try {
    ...
    if (employeeId && scheduleId) {
      ...
      if (typeof day === "string") {
        const schedule = await updateScheduleForEmployee(
          employeeId,
          scheduleId,
          { day, slotStart, slotEnds }
        );
        res.status(201).json(schedule);
      } else res.status(400).json({ message: "Invalid employee ID." });
