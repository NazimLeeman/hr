...
import {
  CheckOutTimeForEmployee,
  createCheckInTimeForEmployee,
  getAllActiveEmployeesAttendance,
} from "../models/attendance/attendance.query";
  ...
  try {
    ...
    if (employeeId && restaurantId) {
      ...
      if (typeof isCheckedIn === "boolean") {
        const position = await createCheckInTimeForEmployee(
          employeeId,
          restaurantId,
          { isCheckedIn }
        );
...
export async function updateAttendanceOfEmployee(req: Request, res: Response) {
  try {
    ...
    if (employeeId && attendanceId) {
      const { isCheckedIn } = req.body;
      if (typeof isCheckedIn === "boolean") {
        const attendance = await CheckOutTimeForEmployee(
          employeeId,
          attendanceId,
          { isCheckedIn }
        );
...
export async function gettingAllActiveEmployees(
  req: AuthRequest,
  res: Response
) {
  try {
