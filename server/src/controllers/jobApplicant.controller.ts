...
export async function applyJob(req: Request, res: Response) {
  try {
    ...
    return res
      .status(201)
      .json({ message: "Application successful.", jobApplicant });
  } catch (error) {
...
export async function getApplicantsForRestaurant(
  req: AuthRequest,
  res: Response
) {
  try {
...
export async function updateJobStatus(req: Request, res: Response) {
  try {
    ...
    if (result[0] === 1) {
      res
        .status(200)
        .json({ success: true, message: "Job applicant updated." });
    } else {
      res
        .status(404)
        .json({ success: false, message: "Job applicant not found ." });
    }
  } catch (error) {
    console.error(error);
    res
      ...
      .json({
        success: false,
        message: "Error while updating job applicant by ID.",
      });
  }
}
