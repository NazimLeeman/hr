...
import {
  ...
  findAllApplicant,
  findApplicantById,
  findApplicantBySearchTerm,
  findAllApplicantLogin,
  deleteApplicantLogin,
} from "../models/applicant/applicant.query";
import {
  createApplicantLogin,
  findApplicantLoginData,
  findApplicantLoginByEmail,
} from "../models/applicantLogin/applicantLogin.query";
      ...
      if (login) {
        if (bcrypt.compareSync(password, login.password)) {
          ...
          if (applicant) {
            ...
            res
              .status(400)
              .send({ message: "This is account is no longer in service." });
        } else
...
export async function getApplicantLoginData(req: Request, res: Response) {
  ...
  try {
    ...
    if (loginData) {
      ...
      return res
        .status(404)
        .json({ message: "Applicant login data not found" });
    }
  } catch (error) {
    console.error(error);
    return res.status(500).json({ message: "Internal Server Error" });
...
export async function addSkillTags(req: Request, res: Response) {
  ...
  try {
    ...
    if (
      existingApplicant &&
      existingApplicant.skillTags &&
      updatedData.skillTags
    ) {
      updatedData.skillTags = [
        ...existingApplicant.skillTags,
        ...updatedData.skillTags,
    ...
    for (const applicantData of applicantsData) {
      ...
      if (
        ...
        if (!loginCheck) {
          ...
          console.log(
            `An account with email ${email} already exists. Skipping...`
          );
        }
      } else {
