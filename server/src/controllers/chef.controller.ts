...
import {
  createEmployeeEfficiency,
  getEmployeeEfficiency,
} from "../models/performance/performance.query";
      ...
      const filteredData = jsonData.filter((entry: any) => {
        const entryDate =
          typeof entry.createdAt === "string"
            ...
            : null;
        return (
          entry.employee !== null && (!entryDate || entryDate >= currentDate)
        );
      });
...
export async function postChefEfficiency(req: Request, res: Response) {
  try {
    ...
    const efficiency = await createEmployeeEfficiency({
      employeeId: chefId,
      orderId,
      servedOnTime,
    });
...
export async function getChefEfficiency(req: Request, res: Response) {
  try {
    const employeeId = Number(req.params.id);
    const efficiency = await getEmployeeEfficiency(employeeId);
    if (efficiency) {
      res.json({ efficiency: efficiency });
    } else {
      res.status(400).json({ message: "Invalid Chef ID." });
